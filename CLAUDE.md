...
## 사진
- `plants/photos/<슬러그>.jpg`(또는 png/webp)를 두면 빌드가 자동으로 리사이즈(640px, 200KB 이하)해 페이지 화분 헤더에 넣는다. 없으면 이모지.
- 현재 사진 일부는 위키미디어 커먼즈의 자유 라이선스 사진 — 출처가 `plants/photos/credits.json` 에 있고 빌드가 `/credits/` 페이지로 표기한다. **credits.json 을 지우면 라이선스 위반.**
- 사장님 실제 사진으로 교체하면: 같은 파일명으로 덮어쓰고 credits.json 에서 해당 슬러그 항목만 삭제.
